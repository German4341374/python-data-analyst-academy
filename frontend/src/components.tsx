  ...
  return (
    <div className="data-viewer">
      <div className="data-title">
        ...
        <span>
          {rows.length} строк × {frame.columns.length} колонок
        </span>
      </div>
      <div className="table-scroll">
                ...
                <th key={column}>
                  <button
                    onClick={() => {
                      setSort({
                        column: index,
                        asc: sort?.column === index ? !sort.asc : true,
                      });
                      setPage(0);
                    }}
                    title="Сортировка только preview"
                  >
                    {column}
                    <ArrowDownUp size={11} />
                  </button>
        ...
        {!rows.length && <p className="empty">Пустая таблица · 0 строк</p>}
      </div>
      <div className="table-footer">
        <span>Сортировка preview не меняет входные данные</span>
        <span>
          <button
  ...
  if (output.image)
    return (
      <figure className="plot">
        <img
          className="rendered-plot"
          src={`data:image/png;base64,${output.image}`}
          alt={`${output.title || "График"}. ${output.xlabel || ""}. ${output.ylabel || ""}.`}
        />
        <figcaption>
          Реальный результат Matplotlib из изолированного runner
        </figcaption>
      </figure>
    );
  const line = output.lines?.[0];
  if (!line || !line.y.length) return <p>На графике нет точек.</p>;
  const min = Math.min(0, ...line.y),
    max = Math.max(1, ...line.y),
    span = max - min || 1;
  ...
  return (
    <figure className="plot">
      ...
      <svg
        viewBox="0 0 700 310"
        role="img"
        aria-label={`${output.title}. ${output.xlabel}: ${line.x.join(", ")}. ${output.ylabel}: ${line.y.join(", ")}`}
      >
        {[0, 1, 2, 3, 4].map((i) => (
          <g key={i}>
            <line
        ...
        <polyline
          ...
          stroke="#b9f58a"
          strokeWidth="3"
          points={line.y.map((v, i) => `${x(i)},${y(v)}`).join(" ")}
        />
        {line.y.map((v, i) => (
          <g key={i}>
            <circle cx={x(i)} cy={y(v)} r="4" fill="#b9f58a" />
            <text
              x={x(i)}
              y="268"
              textAnchor="middle"
      ...
      <p className="muted">
        Preview по реальным точкам и подписям возвращённого Matplotlib Figure.
      </p>
    </figure>
  );
}
